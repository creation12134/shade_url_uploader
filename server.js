...
async function jsonOrThrow(resp, label) {
  if (!resp.ok) {
    ...
    throw new Error(
      `${label} failed: ${resp.status} ${resp.statusText} ${body}`,
    );
  }
  return resp.json();
}
...
async function fetchWithTimeout(
  ...
  const timeoutHandle = setTimeout(() => {
    console.error(
      `[fetch] ${label} TIMED OUT after ${timeoutMs}ms, aborting: ${url}`,
    );
  ...
  try {
    ...
    console.log(
      `[fetch] ${label} -> status=${resp.status} elapsedMs=${Date.now() - startedAt}`,
    );
    return resp;
  } catch (err) {
    console.error(
      `[fetch] ${label} threw after ${Date.now() - startedAt}ms: ${err.message}`,
    );
    throw err;
  } finally {
...
class TokenCacher {
  ...
  async fetchToken() {
    if (
      this.token &&
      this.exp &&
      this.exp - Date.now() / 1000 >= MUST_BE_LONGER_THAN_TO_BE_VALID
    ) {
      return this.token;
    }
    ...
    if (!resp.ok) {
      const body = await resp.text().catch(() => "");
      throw new Error(
        `shade-fs-token failed: ${resp.status} ${resp.statusText} ${body}`,
      );
    }
...
async function makeDirectory(tokenCacher, drive, email, directory) {
  console.log(
    `[mkdir] requesting mkdir drive=${drive} email=${email} directory=${directory}`,
  );
...
async function initiateMultipartUpload(tokenCacher, drive, path, partSize) {
  ...
  if (!result.uploadId) {
    console.warn(
      "[initiate] WARNING: response has no uploadId field — check API response shape above",
    );
  }
  if (!result.token) {
    console.warn(
      "[initiate] WARNING: response has no token (finishToken) field — subsequent calls will fail",
    );
  }
  return result;
}
...
async function presignPart(tokenCacher, drive, partNumber, finishToken) {
  ...
  console.log(
    `[presign] part ${partNumber} presigned url host=${new URL(result.url).host}`,
  );
  return result;
}
...
async function uploadPart(presigned, buffer, partNumber) {
  let lastErr;
  // Debug log: print presigned headers to understand what's expected
  console.log(
    `[upload] presigned headers for part ${partNumber}:`,
    JSON.stringify(presigned.headers || {}),
  );
  for (let attempt = 1; attempt <= MAX_PART_UPLOAD_RETRIES; attempt++) {
    ...
    try {
      ...
      if (!(resp.status >= 200 && resp.status < 300)) {
        throw new Error(
          `UploadPart failed: ${resp.status} ${await resp.text().catch(() => "")}`,
        );
      }
      const etag = resp.headers.get("etag");
      if (!etag) {
        const headerDump = [...resp.headers.entries()]
          .map(([k, v]) => `${k}=${v}`)
          .join(", ");
        throw new Error(
          `Missing ETag on UploadPart response. Headers received: ${headerDump}`,
        );
      }
      console.log(
        `[upload] part ${partNumber} succeeded, etag=${etag}, elapsedMs=${elapsedMs}`,
      );
      return etag;
    } catch (err) {
      const elapsedMs = Date.now() - startedAt;
      lastErr = err;
      console.error(
        `[upload] part ${partNumber} attempt ${attempt} failed after ${elapsedMs}ms: ${err.message}`,
      );
      const backoff = Math.min(1000 * 2 ** (attempt - 1), 15000);
      console.log(
        `[upload] part ${partNumber} backing off ${backoff}ms before retry`,
      );
      await sleep(backoff);
    } finally {
      clearTimeout(timeoutHandle);
...
async function completeMultipart(tokenCacher, drive, finishToken, parts) {
  console.log(
    `[complete] finalizing with ${parts.length} parts: ${JSON.stringify(parts)}`,
  );
  const token = await tokenCacher.fetchToken();
  const url = new URL(
...
async function* chunkStreamIntoParts(readableStream, partSize) {
  ...
  while (true) {
    ...
    if (value) {
      ...
      console.log(
        `[download] read#${readCount} produced no value and not done yet`,
      );
    }
    ...
    while (pendingLen >= partSize) {
      ...
      console.log(
        `[download] assembled full part of ${part.length} bytes, ${pendingLen} bytes carried over`,
      );
      yield part;
    }
    ...
    if (done) {
      ...
      if (pendingLen > 0) {
        ...
        console.log(
          `[download] yielding final partial part of ${finalPart.length} bytes`,
        );
        yield finalPart;
      }
      return;
...
async function transferUrlToShade(
  { sourceUrl, driveId, apiKey, destPath, partSizeBytes },
  onEvent,
) {
  ...
  if (directory && directory !== "/") {
    ...
    log({
      stage: "mkdir",
      message: "destination is at drive root, skipping mkdir",
    });
  }

  log({
    stage: "download-start",
    message: `starting download of ${sourceUrl}`,
  });
  ...
  if (!sourceResp.ok) {
    throw new Error(
      `Failed to fetch sourceUrl: ${sourceResp.status} ${sourceResp.statusText}`,
    );
  }
  if (!sourceResp.body) {
  ...
  if (!finishToken) {
    throw new Error(
      "initiateMultipartUpload did not return a finish token — cannot continue",
    );
  }
  ...
  try {
    for await (const partBuffer of chunkStreamIntoParts(
      sourceResp.body,
      confirmedPartSize,
    )) {
  ...
  log({
    stage: "completing",
    message: `finalizing multipart upload with ${completed.length} parts`,
  });
...
const server = http.createServer(async (req, res) => {
  ...
  console.log(
    `[request] /transfer sourceUrl=${sourceUrl} driveId=${driveId} destPath=${destPath} partSizeMB=${partSizeMB || "default"}`,
  );
  const missing = ["sourceUrl", "driveId", "apiKey", "destPath"].filter(
    (k) => !body[k],
  );
  if (missing.length) {
    res.writeHead(400, { "content-type": "application/json" });
  ...
  const write = (evt) => {
    try {
      ...
      console.error(
        `[response] failed writing to client stream: ${err.message}`,
      );
    }
  };
    ...
    await transferUrlToShade(
      {
        ...
        apiKey,
        destPath,
        partSizeBytes: partSizeMB
          ? Math.round(partSizeMB * 1024 * 1024)
          : undefined,
      },
      write,
    );
...
server.listen(PORT, () => {
  ...
  if (!SERVER_SECRET) {
    console.warn(
      "WARNING: SERVER_SECRET is not set — /transfer is open to anyone who can reach this server.",
    );
  }
});
